...
description: |
  How I read 38,000 PET-CT reports in a matter of minutes.
author:
  - name: Chris McMaster
...
![An example of the prodigy interface.](prodigy.svg "Prodigy in action")
...
![Shapley values from the XGBoost model.](images/paste-35A12A44.png "Shaplet Values")
...
```{r echo=FALSE}
roc <- fromJSON("roc.json")
roc_data <- as.data.frame(roc$data)
names(roc_data) <- roc$columns
...
```{r}
tweetrmd::tweet_embed("https://twitter.com/DrCMcMaster/status/1486592038862655498?s=20&t=pS-zJ7TA2Bk6LDZt9Cpxqg")
```
